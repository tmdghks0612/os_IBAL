...
load (const char *file_name, void (**eip) (void), void **esp) 
{
	...
	token = strtok_r((char*)file_name, DELIM_CHARS, &next_token);
	if(!token){
		return 0;
	}
	while(token != NULL){
		if(!strlcpy(input_word[i],token, strlen(token)+1)){
			return i;
		}
	...
	if (t->pagedir == NULL) 
		goto done;
	process_activate ();
	//when no argument was passed error print
	if(word_count == 0){
		printf("no valid argument passed\n");
		goto done;
	}
	else{
